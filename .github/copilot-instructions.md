...
> **`.github/` に新しいルールファイルを作成してはならない。**

> [!CAUTION]
> 🇺🇸 **This file is a POINTER (table of contents), NOT the rules themselves.**
> All rule definitions live in `axiarch-rules/`. When updating or adding rules, **always edit files inside `axiarch-rules/`**.
> **DO NOT create new rule files in `.github/`.**
...
- **`AGENTS.md`**（プロジェクトルート） — Axiarch System Protocol の最上位プロトコル。全ルールに優先する。
...
1. **`.github/` に新しいルールファイルを作成禁止。** ルールは全て `axiarch-rules/` に配置する。
2. **Universal Rules（`axiarch-rules/{lang}/universal/` 配下の全ファイル）を無断で編集禁止。** 「憲法改正」の明示的指示がある場合のみ許可。
3. **この `copilot-instructions.md` のポインター構造を変更禁止。**
...
#### Step 1: Top-Level Protocol & Master Map (Always Read First)
- **`AGENTS.md`** (project root) — The top-level protocol of the Axiarch System Protocol. Takes precedence over all other rules.
...
1. **DO NOT create new rule files in `.github/`.** All rules belong in `axiarch-rules/`.
...
3. **DO NOT alter the pointer structure of this `copilot-instructions.md`.**
